...
4. `pubmed.sheet_name`이 없다면 `sheets.sheet_name` (없으면 기본 `PubMed`) 탭에 기록됩니다.
...
## 시트 이름
`pubmed.sheet_name`을 설정하면 시트 탭 이름으로 그대로 사용합니다. (예: `Radiology NLP`)

## 복수 쿼리
`pubmed.searches`에 여러 쿼리를 등록하면 각 쿼리별로 지정한 시트에 결과를 기록합니다.
